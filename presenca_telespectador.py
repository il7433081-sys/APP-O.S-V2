...
def alvos_telespectador_de_usuario(usuario: dict[str, Any] | None) -> list[int] | None:
    if not usuario:
        return None
    # Em alguns fluxos (ex.: usuario vindo de _usuario_para_json), os alvos
    # já chegam como lista normalizada em `telespectador_alvos`.
    alvos_lista = usuario.get("telespectador_alvos")
    if isinstance(alvos_lista, list):
        ids: list[int] = []
        for item in alvos_lista:
            try:
                ids.append(int(item))
            except (TypeError, ValueError):
                continue
